...
namespace DB::ast {
    ...
    table::VirtualTable TPTableOp::getOutput()
    {
    ...
    void _tpOutputVisit(std::shared_ptr<const TPBaseOp> root, std::ostream &os, size_t indent)
    {
        tp_op_t_t op_t = root->op_t_;
        std::string prefix = std::string(indent * 2, '-');
        ++indent;
        os << prefix;
        switch (op_t)
        {
        case DB::ast::tp_op_t_t::PROJECT:
        {
			std::shared_ptr<const TPProjectOp> projectOp = std::static_pointer_cast<const TPProjectOp>(root);
            os << "Project on " << std::endl;
            for (auto expr : projectOp->_elements)
            {
                _exprOutputVisit(expr, std::cout, indent);
            }
            os << prefix << "From" << std::endl;
            _tpOutputVisit(projectOp->_source, os, indent);
        }
        break;
        case DB::ast::tp_op_t_t::FILTER:
        {
			std::shared_ptr<const TPFilterOp> filterOp = std::static_pointer_cast<const TPFilterOp>(root);
            os << "Filter with " << std::endl;
            _exprOutputVisit(filterOp->_whereExpr, std::cout, indent);
            os << prefix << "From" << std::endl;
            _tpOutputVisit(filterOp->_source, os, indent);
        }
        break;
        case DB::ast::tp_op_t_t::JOIN:
        {
			std::shared_ptr<const TPJoinOp> joinOp = std::static_pointer_cast<const TPJoinOp>(root);
            ...
            for (auto source : joinOp->_sources)
                _tpOutputVisit(source, os, indent);
        }
        break;
        case DB::ast::tp_op_t_t::TABLE:
        {
			std::shared_ptr<const TPTableOp> tableOp = std::static_pointer_cast<const TPTableOp>(root);
    ...
    check_t_t _tpCheckVisitAtom(std::shared_ptr<const BaseExpr> root, const std::string& tableName)
    {
        ...
        case base_t_t::MATH_OP:
        {
            ...
            check_t_t left_t = _tpCheckVisitAtom(mathPtr->_left, tableName);
            check_t_t right_t = _tpCheckVisitAtom(mathPtr->_right, tableName);
    ...
    void tpCheckVisitAtom(std::shared_ptr<const AtomExpr> root, const std::string tableName)
    {
#ifdef DEBUG
        if (!root)
            throw std::string("AtomExpr* root is nullptr");
#endif // DEBUG
        _tpCheckVisitAtom(root, tableName);
    }

    check_t_t _tpCheckVisit(std::shared_ptr<const BaseExpr> root, const std::string& tableName)
    {
        base_t_t base_t = root->base_t_;
        switch (base_t)
        {
        case base_t_t::LOGICAL_OP:
        {
            ...
            check_t_t left_t = _tpCheckVisit(logicalPtr->_left, tableName);
            check_t_t right_t = _tpCheckVisit(logicalPtr->_right, tableName);
        ...
        case base_t_t::COMPARISON_OP:
        {
            ...
            check_t_t left_t = _tpCheckVisitAtom(comparisonPtr->_left, tableName);
            check_t_t right_t = _tpCheckVisitAtom(comparisonPtr->_right, tableName);
